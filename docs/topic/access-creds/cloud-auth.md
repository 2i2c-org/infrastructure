...
for the projects we _do_ have access to.
...
Access to _all_ projects in this organization can be granted by
...
This lets us use just _one_ set of user credentials for multiple AWS accounts, with
...
4. Prefer _Option 1_ of copying specific environment variables, as that makes it much
   easier to use a new set of credentials when this set expires than using _Option 2_ of
...
1. [Log in to the appropriate account](https://746653422107.signin.aws.amazon.com/console).
   You _must_ already have a traditional IAM account created in this AWS account with
...
2. If asked for the kind of user you are trying to log in as, select _IAM user_.
...
4. For accounts that enforce mandatory Multi-Factor Authentication, we will also
   ...
   Alternatively, the deployer has a convenience command - `aws shell`
   ...
   ```bash
   $ deployer aws shell <aws-profile-name> <arn-of-mfa-device> <code-from-token>
   ```
