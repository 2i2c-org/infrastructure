...
#### 6. Check that the builder docker api and user pods are scheduled on the smallest available instance
...
```{important}
For aws, you can start from the `config/clusters/template/aws/enc-binder.secret.values.yaml` and update whatever feels appropriate.
```
