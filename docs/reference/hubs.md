...
% NOTE: The CSV files used by this page are generated by `docs/helper_programs/hub_info_table.py`
...
```{admonition} About kubeconfig hubs
:class: dropdown
Hubs that have `kubeconfig` listed as their provider mean we are configuring Kubernetes and it's harder to infer the provider.
We may lack some information about these hubs, but do our best to infer it from the configuration.
```
