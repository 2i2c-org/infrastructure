...
def deploy(cluster_name, hub_name, test_notebook_path):
    """
    ...
    if cluster_name:
        ...
        with cluster.auth():
            ...
            if hub_name:
                ...
                hub.deploy(k, PROXY_SECRET_KEY, test_notebook_path)
            else:
                for hub in hubs:
                    hub.deploy(k, PROXY_SECRET_KEY, test_notebook_path)
    else:
...
def main():
    ...
    if args.action == 'build':
        ...
        deploy(args.cluster_name, args.hub_name, args.test_notebook_path)
